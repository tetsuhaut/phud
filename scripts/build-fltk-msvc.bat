...
SET ZIP_NAME_FROM_FLTK_WEB_SITE=%LIB_NAME%-%VERSION%-source
REM SET ZIP_NAME_FROM_FLTK_WEB_SITE=%LIB_NAME%-%VERSION%
...
SET ZIP_NAME_FROM_FLTK_GITHUB=release-%VERSION%
...
SET ZIP_NAME_FROM_FLTK_GITHUB=%LIB_NAME%-release-%VERSION%
IF EXIST %ZIP_NAME_FROM_FLTK_GITHUB%.zip (
  7z x %ZIP_NAME_FROM_FLTK_GITHUB%.zip
  SET SRC_DIR=%LIB_NAME%-release-%VERSION%
  GOTO :build
) ELSE (
  ECHO Could not find the archive file %ZIP_NAME_FROM_FLTK_GITHUB%.tar.gz
)
