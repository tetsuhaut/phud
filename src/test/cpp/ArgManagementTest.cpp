...
[[nodiscard]] static int myStaticFunction(int i) { return i; }
...
BOOST_AUTO_TEST_SUITE(ArgManagementTest)

BOOST_AUTO_TEST_CASE(ArgManagementTest_hideArgsShouldSucceed) {
  const int anInt { 3 };
...
BOOST_AUTO_TEST_CASE(ArgManagementTest_hideArgsAndRetrieveInAnotherThreadShouldSucceed) {
  const int anInt { 3 };
  const double aDouble { 4.0 };
  const String aString { "toto" };
  const std::function<int(int)> aFunction { myStaticFunction };
  ...
  hideArgs(anInt, aDouble, aString, aFunction))
  .join();
  BOOST_REQUIRE(42 == retrievedFctPtrWasCalled.load());
}

BOOST_AUTO_TEST_CASE(ArgManagementTest_hideMemberFunctionAndRetrieveInAnotherThreadShouldSucceed) {
  MyStruct anObject;
