...
BOOST_AUTO_TEST_CASE(TimeTest_timeShouldBeInvalidByDefault) {
  BOOST_CHECK_THROW(std::ignore = Time({.strTime = "", .format = WINAMAX_HISTORY_TIME_FORMAT}),
                    TimeException);
  BOOST_CHECK_THROW(std::ignore = Time({.strTime = "", .format = PMU_HISTORY_TIME_FORMAT}),
                    TimeException);
}

BOOST_AUTO_TEST_CASE(TimeTest_timeShouldBeCreatedWithACorrectFormat) {
  BOOST_CHECK_THROW(std::ignore =
                        Time({.strTime = "2014/10/31 00:45:01", .format = PMU_HISTORY_TIME_FORMAT}),
                    TimeException);
...
BOOST_AUTO_TEST_CASE(TimeTest_timeToStringShouldSucceed) {
  ...
  const Time time2(
      {.strTime = "Tuesday, September 14, 18:33:39 2021", .format = PMU_HISTORY_TIME_FORMAT});
  BOOST_REQUIRE("2021-09-14 18:33:39" == time2.toSqliteDate());
}

BOOST_AUTO_TEST_CASE(TimeTest_constructingTimeByCopyShouldSucceed) {
  const auto time1 {
      Time({.strTime = "2014/10/31 00:45:01", .format = WINAMAX_HISTORY_TIME_FORMAT})};
  BOOST_REQUIRE("2014-10-31 00:45:01" == time1.toSqliteDate());
  const auto time2 {
      Time({.strTime = "Tuesday, September 14, 18:33:39 2021", .format = PMU_HISTORY_TIME_FORMAT})};
  BOOST_REQUIRE("2021-09-14 18:33:39" == time2.toSqliteDate());
}
