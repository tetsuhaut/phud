...
static std::string utf16To8(std::wstring_view utf16String) {
  std::string ret;
...
BOOST_AUTO_TEST_CASE(HandTest_loadingDoubleOrNothingWithOnlyFoldsShouldSucceed) {
  ...
  BOOST_REQUIRE_MESSAGE(Time({.strTime = "2014/10/31 00:45:01",
                              .format = WINAMAX_HISTORY_TIME_FORMAT}) == t->getStartDate(),
...
BOOST_AUTO_TEST_CASE(HandTest_parsingSuperFreeRollTournamentFileNameShouldSucceed) {
  const auto [isRealMoney, gameName, variant,
              limit] {parseFileStem(utf16To8(L"20150630_Super Freeroll Stade 1 - Déglingos _"
                                             "(123322389)_real_holdem_no-limit"))
                          .value()};
  BOOST_REQUIRE_MESSAGE(utf16To8(L"Super Freeroll Stade 1 - Déglingos _(123322389)") == gameName,
                        "gameName=" << gameName);
  BOOST_REQUIRE(true == isRealMoney);
  BOOST_REQUIRE(Variant::holdem == variant);
  BOOST_REQUIRE(Limit::noLimit == limit);
...
BOOST_AUTO_TEST_CASE(HandTest_parsingOmaha5CashgameFileNameShouldSucceed) {
  const auto [isRealMoney, gameName, variant,
              limit] {parseFileStem("20180304_Ferrare 02_play_omaha5_pot-limit").value()};
...
BOOST_AUTO_TEST_CASE(HandTest_parsingShortTrackCashgameFileNameShouldSucceed) {
  const auto [isRealMoney, gameName, variant, limit] {
      parseFileStem(utf16To8(L"20180304_SHORT TRACK 0,10€_real_holdem_no-limit")).value()};
...
BOOST_AUTO_TEST_CASE(HandTest_buildCashGameHandShouldSucceed) {
  const pt::TmpFile file {"20150718_Hold'em(123703551)_play_holdem_no-limit.txt"};
  file.printLn("Winamax Poker - Tournament "
               "Hold'em"
               " buyIn: 4,50 + 0,50 level: 17 - HandId: #531302705944068104-65-1437230557 - Holdem "
               "no limit (160/800/1600) - 2015/07/18 14:42:37 UTC");
...
BOOST_AUTO_TEST_CASE(HandTest_buildTournamentHandOneRaiseBbIsAllInShouldSucceed) {
  ...
  BOOST_REQUIRE(playerNames[9].empty());
...
BOOST_AUTO_TEST_CASE(HandTest_playerWithNoActionHasActionNone) {
  const auto [isRealMoney, gameName, variant, limit] {
      parseFileStem("20150630_Super Freeroll Stade 1 - Déglingos _(123322389)_real_holdem_no-limit")
          .value()};
