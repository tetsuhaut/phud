...
class EnumMapper {
 ...
 public:
  ...
  [[nodiscard]] constexpr std::string_view toString(EnumType e) const {
    const auto it {
        std::ranges::find_if(m_enumToStringList, [e](const auto& p) { return p.first == e; })};
    return (it != m_enumToStringList.end())
               ? it->second
               : throw std::invalid_argument("Invalid enum type value");
  }

  [[nodiscard]] constexpr EnumType fromString(std::string_view s) const {
    const auto it {
        std::ranges::find_if(m_enumToStringList, [s](const auto& p) { return p.second == s; })};
    return (it != m_enumToStringList.end())
               ? it->first
...
[[nodiscard]] constexpr auto makeEnumMapper(std::pair<EnumType, std::string_view> first,
                                            Args... rest) {
  constexpr auto N {1 + sizeof...(Args)};
  return EnumMapper<EnumType, N>(
      std::array<std::pair<EnumType, std::string_view>, N> {first, rest...});
}
