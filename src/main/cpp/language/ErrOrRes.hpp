...
class [[nodiscard]] ErrOrRes final {
 private:
   ...
   * builds an result.
   */
  explicit constexpr ErrOrRes(RESULT r)
    : m_err {},
      m_res {r} {}
  /**
   ...
   */
  constexpr ErrOrRes(std::in_place_t, std::string_view s)
    : m_err {s},
      m_res {} {}
 ...
 public:
  ...
  [[nodiscard]] static constexpr ErrOrRes err(std::string_view s) {
    return ErrOrRes(std::in_place, s);
  }
