...
struct [[nodiscard]] Gui::Implementation final {
  ...
  std::unordered_map<std::string,
                     std::array<std::unique_ptr<PlayerIndicator>, TableConstants::MAX_SEATS>>
      m_playerIndicators {};
...
namespace {
  ...
  using TablePlayerIndicators =
      std::array<std::unique_ptr<PlayerIndicator>, TableConstants::MAX_SEATS>;
  using TableWindowTitleToTablePlayerIndicators =
      std::unordered_map<std::string, TablePlayerIndicators>;
  ...
  /**
   * Schedules a function to be executed by the main GUI thread during the next message handling
   * cycle.
  ...
  void onImportProgress(Fl_Progress* progressBar) {
    scheduleUITask([pb = progressBar]() {
      pb->value(pb->value() + 1);
      pb->copy_label(
          fmt::format("{}/{}", limits::toInt(pb->value()), limits::toInt(pb->maximum())).c_str());
    });
  }
  ...
  void onImportDone(Fl_Button* chooseHistoDirBtn) {
    scheduleUITask([chb = chooseHistoDirBtn]() { chb->activate(); });
  }
  ...
  template <typename WIDGET>
  [[nodiscard]] gsl::not_null<WIDGET*> createWidget(const phud::Rectangle& bounds,
                                                    std::string_view label, auto setupFunction) {
    ...
    for (const auto& seat : seats) {
      ...
      if (auto ps {tableStatistics.extractPlayerStatistics(seat)}; nullptr != ps) {
        LOG().debug<"Creating/updating indicator for player '{}' at seat {}">(ps->getPlayerName(),
                                                                              seatStr);
        const auto rotatedSeat =
            gui::rotateRelativeToHero(seat, heroSeat, tableStatistics.getMaxSeat());
        const auto pos = gui::buildPlayerIndicatorPosition(
            rotatedSeat, tableStatistics.getMaxSeat(), tablePosition);
  ...
  void
  removeUselessPlayerIndicators(TableWindowTitleToTablePlayerIndicators& tableToPlayerIndicators,
  ...
  TableService::TableObserverCallback
  buildTableObserver(TableWindowTitleToTablePlayerIndicators& playerIndicators,
  ...
  void updateUsefulPlayerIndicators(TableWindowTitleToTablePlayerIndicators& playerIndicators,
                                    const std::span<const std::string> tableWindowTitles,
                                    TableService& tableService) {
    LOG().info<"Create/Update table indicators for {} table(s)">(tableWindowTitles.size());
    const auto& tableTitleNotYetMonitored {
        tableWindowTitles | std::views::filter([&playerIndicators](const auto& title) {
          return !playerIndicators.contains(title);
        }) |
   ...
   */
  void importDirAwakeCb(Fl_Progress* progressBar, Fl_Button* chooseHistoDirBtn,
                        HistoryService& historyService, const fs::path& dir) {
    LOG().debug<__func__>();
  ...
  namespace DirectoryChoiceHandler {
    ...
    void handleOk(std::string_view dirName, Gui::Implementation& self) {
      ...
      if (self.m_historyService.isValidHistory(dir)) {
        ...
        scheduleUITask([pb = self.m_progressBar, chb = self.m_chooseHistoDirBtn,
                        historyService = &self.m_historyService, dir]() {
          try {
  ...
  void handleDirectoryChoice(Gui::Implementation& self,
                             const Fl_Native_File_Chooser& dirChoser, FileChoiceStatus fcs) {
    switch (fcs) {
      case FileChoiceStatus::ok:
        DirectoryChoiceHandler::handleOk(dirChoser.filename(), self);
        return;
      case FileChoiceStatus::error:
        [[unlikely]] DirectoryChoiceHandler::handleError(dirChoser, self);
        return;
      case FileChoiceStatus::cancel: /* nothing to do */
        return;
    }
    std::unreachable();
  }

  [[nodiscard]] gsl::not_null<Fl_Button*> buildChooseHistoDirBtn(Gui::Implementation& aSelf) {
    // Called by the GUI (button component) when the user clicks on the 'choose history directory'
    ...
    return createWidget<Fl_Button>(MainWindow::Screen::chooseHistoDirBtn,
                                   ...
                                   [&aSelf, choseHistoDirCb](Fl_Button* btn) {
                                     /* setup the button for choosing which history directory to
                                      * import into database */
                                     btn->callback(choseHistoDirCb, &aSelf);
                                   });
  }

  [[nodiscard]] gsl::not_null<Fl_Button*> buildStopHudBtn(Gui::Implementation& aSelf) {
    ...
    return createWidget<Fl_Button>(MainWindow::Screen::stopHudBtn, MainWindow::Label::stopHud,
  ...
  [[nodiscard]] gsl::not_null<Fl_Box*> buildHistoDirTextField(const Preferences& pref) {
  ...
  [[nodiscard]] gsl::not_null<Fl_Progress*> buildProgressBar() {
  ...
  [[nodiscard]] gsl::not_null<Fl_Box*> buildWatchedTablesLabel() {
    return createWidget<Fl_Box>(MainWindow::Screen::watchedTableLabel,
                                MainWindow::Label::noPokerTableDetected,
                                [](Fl_Box* box) { box->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE); });
  }

  [[nodiscard]] gsl::not_null<Fl_Box*> buildInfoBar() {
    return createWidget<Fl_Box>(MainWindow::Screen::infoBar, MainWindow::Label::WELCOME);
  }

  std::string getWatchedTableLabel(std::span<const std::string> tableNames) {
    if (tableNames.empty()) {
  ...
  [[nodiscard]] std::unique_ptr<TableWatcher>
  ...
  gsl::not_null<MyMainWindow*> buildMainWindow(Gui::Implementation* impl) {
...
Gui::Gui(TableService& tableService, HistoryService& historyService)
  ...
  if (const auto dir {m_pImpl->m_preferences->getPreferredHistoDir()};
      PokerSiteHistory::isValidHistory(dir)) {
  ...
  m_pImpl->m_tableWatcher = buildTableWatcher(m_pImpl->m_watchedTablesLabel,
                                              m_pImpl->m_playerIndicators, m_pImpl->m_tableService);
...
// TODO : remplacer le choix du répertoire d'historique à charger par un menu listant tous les
// répertoires connus
// TODO : remplacer les periodic task des StatProducer/StatReader par une requête exécutée si le
// fichier d'historique de la table change
/*
