...
void informUser(Gui::Implementation& aSelf) {
  LOG().debug<__func__>();
  scheduleUITask(
      [infoBar = aSelf.m_infoBar, msg = std::string(MSG)]() { infoBar->copy_label(msg.c_str()); });
}
...
void informUser(Gui::Implementation& aSelf, std::string_view aMsg) {
  LOG().debug<__func__>();
  scheduleUITask(
      [infoBar = aSelf.m_infoBar, msg = std::string(aMsg)]() { infoBar->copy_label(msg.c_str()); });
}
...
void updateTablePlayerIndicators(TablePlayerIndicators& playerIndicators,
  ...
  for (const auto& seat : seats) {
    ...
    if (auto ps {tableStatistics.extractPlayerStatistics(seat)}; nullptr != ps) {
      ...
      const auto pos = gui::buildPlayerIndicatorPosition(rotatedSeat, tableStatistics.getMaxSeat(),
                                                         tablePosition);
...
void removeUselessPlayerIndicators(TableWindowTitleToTablePlayerIndicators& tableToPlayerIndicators,
...
void updateUsefulPlayerIndicators(TableWindowTitleToTablePlayerIndicators& playerIndicators,
  ...
  const auto& tableTitleNotYetMonitored {tableWindowTitles |
                                         std::views::filter([&playerIndicators](const auto& title) {
...
static void handleDirectoryChoice(Gui::Implementation& self,
...
[[nodiscard]] static gsl::not_null<Fl_Button*> buildChooseHistoDirBtn(Gui::Implementation& aSelf) {
...
[[nodiscard]] static std::unique_ptr<TableWatcher>
