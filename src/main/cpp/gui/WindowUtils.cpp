...
//   char err[MAX_PATH + 1] {};
//   const auto size {
//     FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, GetLastError(), localeId, &err[0],
//     MAX_PATH, nullptr)
//   };
//   err[MAX_PATH] = '\0';
...
//   const auto myProcessHandle { OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE,
//   pid) }; validation::requireNotNull(myProcessHandle, getLastErrorMessageFromOS()); const auto _
//   { gsl::finally([myProcessHandle] { CloseHandle(myProcessHandle); }) }; char process[MAX_PATH +
//   1] {};
//
//   if (const auto nbChars { GetModuleFileNameEx(myProcessHandle, nullptr, &process[0], MAX_PATH)
//   }; 0 != nbChars) {
//     process[MAX_PATH] = '\0';
//     return &process[0];
//   }
...
BOOL windowTitleCallback(HWND hwnd, LPARAM hiddenTitles) {
  ...
  if (IsWindowVisible(hwnd)) {
    ...
    if (const auto actualLength {GetWindowText(hwnd, title.data(), lengthPlusOne)};
        actualLength > 0) {
...
std::optional<phud::Rectangle>
mswindows::getTableWindowRectangle(std::string_view tableWindowTitle) {
