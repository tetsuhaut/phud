...
class [[nodiscard]] PokerSiteHistory {
 ...
 public:
   ...
   */
  [[nodiscard]] virtual std::unique_ptr<Site>
  load(const std::filesystem::path& historyDir, std::function<void()> onProgress,
  ...
  [[nodiscard]] virtual std::unique_ptr<Site>
  reloadFile(const std::filesystem::path& winamaxHistoryFile) = 0;
  ...
  getTableNameFromTableWindowTitle(std::string_view tableWindowTitle) const = 0;
  [[nodiscard]] virtual std::optional<std::filesystem::path>
  getHistoryFileFromTableWindowTitle(const std::filesystem::path& historyDir,
                                     std::string_view tableWindowTitle) const = 0;
  // force the use of std::filesystem::path
  std::optional<std::filesystem::path>
  getHistoryFileFromTableWindowTitle(auto historyDir,
