...
// case Street::preflop: return { previous.at(0), previous.at(1), previous.at(2), previous.at(3),
// previous.at(4) }; case Street::flop: return { cards.at(0), cards.at(1), cards.at(2),
// previous.at(3), previous.at(4) }; case Street::turn: return { previous.at(0), previous.at(1),
// previous.at(2), cards.at(3), previous.at(4) }; case Street::river: return { previous.at(0),
// previous.at(1), previous.at(2), previous.at(3), cards.at(4) };
// }
...
// return std::string { line.substr(HAND_HISTORY_FOR_GAME_SIZE, line.rfind(" *****") -
// HAND_HISTORY_FOR_GAME_SIZE) };
// }

// [[nodiscard]] bool isDigit(const auto& someGenericChar) { return 0 !=
// std::isdigit(someGenericChar); }
...
//  const auto smallBlind { ps::toAmount(line.substr(smallBlindPos, bigBlindPos - smallBlindPos -
//  1)) }; const auto bigBlind { ps::toAmount(line.substr(bigBlindPos, line.find(' '))) }; const
//  auto limit { ps::contains(line, " NL ") ? Limit::noLimit : Limit::potLimit }; const auto variant
//  { ps::contains(line, " Texas Hold'em ") ? Variant::holdem : Variant::omaha }; std::string
//  dateStr { line.substr(line.find(" - ") + ps::length(" - ")) };
...
//  return {
//  .m_smallBlind = smallBlind, .m_bigBlind = bigBlind, .m_startDate = date, .m_limit = limit,
//  .m_variant = variant
// };
// }
...
//"^Dealt to (.*) \\[(.*)\\]$"
//  const auto playerName { line.substr(DEALT_TO_LENGTH, line.find(' ', DEALT_TO_LENGTH) -
//  DEALT_TO_LENGTH) }; cache.setIsHero(playerName); return parseCards(line, " ");
// }
...
//  ret = { .m_playerName = line.substr(0, line.rfind(' ')), .m_type = ActionType::fold, .m_bet =
//  0.0 };
// }
// else if (line.ends_with(" checks")) {
// ret = { .m_playerName = line.substr(0, line.rfind(' ')), .m_type = ActionType::check, .m_bet =
// 0.0 };
// }
...
// [[nodiscard]] static std::array<std::string, TableConstants::MAX_SEATS> parseWinners(TextFile&
// tf) { std::array<std::string, TableConstants::MAX_SEATS> winners; auto pos { std::string::npos };
// std::size_t i { 0 };
...
// const auto& [nbMaxSeats, tableName, buttonSeat] {
// getNbMaxSeatsTableNameButtonSeatFromTableLine(tf) }; const auto& seatPlayers { parseSeats(tf,
// cache) };
...
// .tableName = tableName, .buttonSeat = tableSeat::fromInt(buttonSeat), .maxSeats =
// tableSeat::fromInt(nbMaxSeats), .level = level, .ante = ante, .startDate = date, .seatPlayers =
// seatPlayers, .heroCards = heroCards, .boardCards = boardCards, .actions = std::move(actions),
// .winners = winners
// };
...
// LOG().debug<"nb actions={}">(actions.size());
// Hand::Params p {
// .id = handId, .gameType = GameType::cashGame, .siteName = ProgramInfos::PMU_SITE_NAME, .tableName
// = tableName, .buttonSeat = buttonSeat, .maxSeats = nbMaxSeats, .level = 0, .ante = 0, .startDate
// = startDate, .seatPlayers = seatPlayers, .heroCards = heroCards, .boardCards = boardCards,
// .actions = std::move(actions), .winners = winners
// };
...
// const auto handId { readHandId(tf.getLine()) };
// tf.next();
// const auto& [smallBlind, bigBlind, startDate, limit, variant] {
// getInfosFromCashGamePmuPokerLine(tf.getLine()) }; tf.next(); const auto& [tableName, isRealMoney]
// { parseTableLine(tf.getLine()) }; tf.next(); const auto buttonSeat {
// parseButtonSeatLine(tf.getLine()) }; tf.next(); const auto nbMaxSeats {
// parseTotalNumberOfPlayersLine(tf.getLine()) }; tf.next(); const auto& seatPlayers {
// parseSeats(tf, pc) }; seekToDealingDownCards(tf); tf.next(); const auto& heroCards {
// parseHeroCards(tf.getLine(), pc) }; auto [actions, winners, boardCards] {
// parseActionsAndWinnersAndBoardCards(tf, handId) }; LOG().debug<"nb actions={}">(actions.size());
// Hand::Params params {
// .id = handId, .gameType = GameType::cashGame, .siteName = ProgramInfos::PMU_SITE_NAME, .tableName
// = tableName, .buttonSeat = buttonSeat, .maxSeats = nbMaxSeats, .level = 0, .ante = 0, .startDate
// = startDate, .seatPlayers = seatPlayers, .heroCards = heroCards, .boardCards = boardCards,
...
// const auto& [buyIn, level, date, handId] {
// getBuyInLevelDateHandIdFromTournamentWinamaxPokerLine(tf.getLine()) }; auto pHand {
// getHand<GameType::tournament>(tf, pc, level, date, handId) }; return { std::move(pHand),
// std::make_unique<GameData>(pHand->getStartDate(), buyIn, 0, 0, pHand->getMaxSeats()) };
