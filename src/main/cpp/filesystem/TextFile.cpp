...
bool TextFile::containsOneOf(std::span<const std::string_view> patterns) const {
  return std::end(patterns) != std::ranges::find_if(patterns, [this](const auto& s) noexcept {
           return containsExact(s);
         });
}
...
int TextFile::getLineIndex() const noexcept {
  return m_lineNb;
}
std::string TextFile::getFileName() const {
  return m_file.string();
}
std::string TextFile::getFileStem() const {
...
bool TextFile::startsWith(char c) const noexcept {
  return m_line.starts_with(c);
}
bool TextFile::startsWith(std::string_view s) const noexcept {
  return m_line.starts_with(s);
}
bool TextFile::endsWith(std::string_view s) const noexcept {
  return m_line.ends_with(s);
}
bool TextFile::endsWith(char c) const noexcept {
  return m_line.ends_with(c);
}
